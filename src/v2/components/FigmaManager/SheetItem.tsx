  ...
  return (
    <div key={sheet.spreadsheetId} className="mb-4">
      <div className="flex mb-2 items-center gap-2">
        ...
        <div
          className="flex-grow p-2 border flex items-center justify-between"
        >
          <span>
            {sheet.sheetName}
          </span>
        ...
        <button
          className="border p-2"
          onClick={() => onRemove(sheet.spreadsheetId)}
        >
          ✖
        </button>
      </div>
                  ...
                  <Tooltip.Root key={color.hex + index}>
                    <Tooltip.Trigger asChild>
                      <div
                        ...
                        onClick={() => onColorClick(color, {sheetId: sheet.sheetId, sheetName: sheet.sheetName, spreadsheetId: sheet.spreadsheetId}, index+1)}
                      />
                    </Tooltip.Trigger>
                    <Tooltip.Portal>
                      <Tooltip.Content
                      ...
                      >
                        <div className="flex flex-col gap-1">
                          ...
                          {color.slashNaming && <div>Slash Naming: {color.slashNaming}</div>}
                          {color.comments && <div>Comments: {color.comments}</div>}
                          {color.additionalColumns.map((column) => (
                            <div key={column.name}>{column.name}: {column.value}</div>
                          ))}
                        </div>
